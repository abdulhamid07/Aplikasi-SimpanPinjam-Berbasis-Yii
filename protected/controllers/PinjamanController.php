...
class PinjamanController extends Controller
{
			...
			{		
				$model->save();	
					for($i=1; $i<=$jum; $i++)
					{
						...
						$con->createCommand()->insert('angsuran',array(
				...
				$transaksi=rollBack();
				Yii::app()->user->setFlash('error','Terjadi Kesalahan,penyimpanan gagal');
			}
		}
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Pinjaman']))
		{
			...
			try
			{	
				//Hapus data angsuran lama	
				$con->createCommand("DELETE FROM angsuran WHERE pinjaman_id=".$id)->execute();
				$model->save();
